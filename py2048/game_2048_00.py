...
class Window(pyglet.window.Window):
    ...
    def game_init(self):
        ...
        # 悔棋
        self.undo_label = pyglet.text.Label(text="please use U to undo!you can undo %d times."%(len(self.buffer)),
                                            bold=True, color=(119, 110, 101, 255), x=STARTX, y=60,
                                            font_size=16, batch=self.main_batch)
        # restart
        self.restart_label = pyglet.text.Label(text="please use R to replay,ESC to quit!", bold=True,
    ...
    def on_draw(self):
        ...
        self.undo_label.text = "please use U to undo!you can undo %d times."%(len(self.buffer))
    ...
    def draw_tile(self, xywh, data):
        ...
        if data != 0:
            a = pyglet.text.Label(text=str(data), bold=True, anchor_x='center', anchor_y='center',
                                  color=(0, 0, 0, 255), x=x+dx/2, y=y+dy/2, font_size=20)
            a.draw()
    ...
    def on_key_press(self, symbol, modifiers):
        ...
        if symbol == key.UP:
            print("up press")
            self.data, eq_title, score = self.slideUpDown(True)
            key_press = True
        elif symbol == key.DOWN:
            print("down press")
            self.data, eq_title, score = self.slideUpDown(False)
            key_press = True
        elif symbol == key.LEFT:
            print("left press")
            self.data, eq_title, score = self.slideLeftRight(True)
            key_press = True
        elif symbol == key.RIGHT:
            print("right press")
            self.data, eq_title, score = self.slideLeftRight(False)
            key_press = True
        ...
        if key_press and (not self.put_tile()):
            ...
            if a and b and c and d:
                print("Game Over")
                over = pyglet.text.Label(text="你输了！\n请重新开始", bold=True, anchor_x='center', anchor_y='center',
                                      color=(255, 255, 205, 255), x=WIN_WIDTH/2, y=WIN_HEIGHT/2,
                                      font_size=30, batch=self.main_batch)


        if key_press and (not eq_title):
